...
@property (nonatomic, strong) MPOImage *image;
...
- (void)loadView {
    ...
    self.scrollView.backgroundColor = [UIColor dimensivaBackgroundColor];
    self.view = self.scrollView;
    
    UIImageView *photoImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0.0f, 42.0f, 320.0f, 320.0f)];
    [photoImageView setBackgroundColor:[UIColor blackColor]];
    UIImage *img = [[image getUIImageAtIndex:0] resizedImageByMagick:@"560x560#"];
...
- (BOOL)shouldUploadImage:(MPOImage *)anImage {
...
//    UIImage *thumbnailImage = [anImage thumbnailImage:86.0f transparentBorder:0.0f cornerRadius:10.0f interpolationQuality:kCGInterpolationDefault];
    ...
    self.photoFile = [PFFile fileWithName:[anImage getAutoGeneratedFilename]  data:imageData];
    ...
    NSLog(@"Requested background expiration task with id %lu for Dimensiva photo upload", (unsigned long)self.fileUploadBackgroundTaskId);
...
- (void)doneButtonAction:(id)sender {
    ...
    [photo setObject:[PFUser currentUser] forKey:kPAPPhotoUserKey];
        ...
        [photo setObject:self.photoFile forKey:@"picture"];
        [photo setObject:self.thumbnailFile forKey:kPAPPhotoThumbnailKey];
        photo[@"title"] = [image getAutoGeneratedFilename];
    ...
    self.photoPostBackgroundTaskId = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
        ...
        [photo saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
            if (succeeded) {
                ...
                [[PAPCache sharedCache] setAttributesForPhoto:photo authorName:[[PFUser currentUser] username] likers:[NSArray array] commenters:[NSArray array] likedByCurrentUser:NO];
